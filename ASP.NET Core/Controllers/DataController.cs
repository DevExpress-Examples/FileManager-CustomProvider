...
public class DataController : Controller {
    ...
    public DataController(IWebHostEnvironment env) {
        rootFolderName = Path.Combine(env.ContentRootPath, "RootFolder");
    }
    ...
    public FileResult DownloadItems(string name) {
        if (string.IsNullOrEmpty(name))
            return null;
        var items = JsonSerializer.Deserialize<string[]>(name);
        if (items.Length == 1) {
    ...
    public string MoveItem(string pathInfo, string destinationDirectory) {
        ...
        if (Directory.Exists(pathInfo)) {
            ...
            if (targetDirectory.GetDirectories().FirstOrDefault(d => d.Name == sourceDirectory.Name) != null) {
                return this.CreateResponse(false, 409, "The directory already exists in the destination folder", null);
            } else {
    ...
    public string UploadChunk(IFormFile fileChunk) {
        if (Request.HasFormContentType) {
            ...
            var chunkMetadata = JsonSerializer.Deserialize<ChunkMetadata>(metadataSerialized.ToString(), jsonOptions);
            var destinationDirectory = JsonSerializer.Deserialize<FileDataItem>(destinationDirSerialized.ToString(), jsonOptions);
